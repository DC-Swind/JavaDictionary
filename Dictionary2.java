...
public class Dictionary extends JFrame{
	...
	class InputValueChanged implements DocumentListener {
        ...
        public void update(){
            ...
            RightArea.setText(searchAns.meanans);
        }
	}
    ...
    class AddListener implements ActionListener{
        public void actionPerformed(ActionEvent e){
            ...
            //JOptionPane getinfo = new JOptionPane();
            String inputWord = InputMessage.getText();
            String inputMean = JOptionPane.showInputDialog("Please input mean");
            String inputYinbiao = JOptionPane.showInputDialog("Please input yinbiao");
            System.out.println(inputMean +" "+ inputYinbiao);
            ...
            try {     
                ...
                writer.write("\t");
                writer.write(inputYinbiao);
                writer.write("\t");
                writer.write(inputMean);                
                writer.close();
            ...
            wordlist.DicTree.insert(inputWord,inputYinbiao,inputMean);
            wordlist.BkTree.insert(inputWord,inputYinbiao,inputMean);
			wordlist.currentsize++;
        }   
	...
	Dictionary(){
        ...
        */
        JButton AddNew = new JButton("Add this word");
        DownField.add(AddNew);
...
class SearchWords{
    ...
    SearchWords(final ReadDictionary wordlist,String word){
        ...
        if (word.length() > 2){
            ...
            for(int i =0; i<slen-1; i++) if (wordlist.DicTree.search(sentence[i]+" "+sentence[i+1])[0] != null){
                sentence[i] = sentence[i] + " "+ sentence[i+1];
                sentence[i+1] = ""; 
            }
            
            for(int i =0; i<slen; i++){
                TreeNode[] ansnodes2 = wordlist.DicTree.search(sentence[i]);
                if (ansnodes2[0] != null )meanans = meanans +"  "+ ansnodes2[0].mean;
            }
        }
        
        System.out.println(meanans);
    }
}
